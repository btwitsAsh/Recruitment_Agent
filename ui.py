...
def display_analysis_results(analysis_result):
    ...
    evaluation_mode = analysis_result.get("mode", "ROLE_MODE")
    selected = analysis_result.get("selected", False)
    decision_band = analysis_result.get("decision_band", "")
    skill_scores = analysis_result.get("skill_scores", {})
    detailed_weaknesses = analysis_result.get("detailed_weaknesses", [])
    qualitative_feedback = analysis_result.get("qualitative_feedback", {})
    ...
    mode_color = "#1565C0" if evaluation_mode == "JD_MODE" else "#6A1B9A"
    mode_label = "JD-Based Evaluation" if evaluation_mode == "JD_MODE" else "Role-Based Evaluation"
    st.markdown(
        f"<div style='margin-bottom: 12px;'><span style='background:{mode_color}; color:white; "
    ...
    with col2:
        if decision_band == "strong_hire":
            st.markdown("<h2 style='color:#2e7d32;'>✅ Strong Hire (80+)</h2>", unsafe_allow_html=True)
        elif decision_band == "shortlist" or selected:
            st.markdown("<h2 style='color:#4CAF50;'>✅ Shortlisted (70-79)</h2>", unsafe_allow_html=True)
        elif decision_band == "borderline":
            st.markdown("<h2 style='color:#f9a825;'>⚠️ Borderline / Maybe (60-69)</h2>", unsafe_allow_html=True)
        else:
            st.markdown("<h2 style='color:#d32f2f;'>❌ Not Selected (<60)</h2>", unsafe_allow_html=True)
        st.write(analysis_result.get('reasoning', ''))

        # Optional qualitative recruiter layer
        if qualitative_feedback:
            st.markdown("<hr>", unsafe_allow_html=True)
            st.subheader("🧠 Recruiter Qualitative Review")
            verdict = qualitative_feedback.get("verdict", "borderline").title()
            confidence = qualitative_feedback.get("confidence", "medium").title()
            st.markdown(
                f"<p><strong>Verdict:</strong> {verdict} &nbsp;|&nbsp; <strong>Confidence:</strong> {confidence}</p>",
                unsafe_allow_html=True,
            )

            summary = qualitative_feedback.get("summary", "")
            if summary:
                st.markdown(f"<p><strong>Summary:</strong> {summary}</p>", unsafe_allow_html=True)

            recruiter_comments = qualitative_feedback.get("recruiter_comments", [])
            if recruiter_comments:
                st.markdown("<strong>Recruiter comments:</strong>", unsafe_allow_html=True)
                for comment in recruiter_comments:
                    st.markdown(f"- {comment}")

            top_improvements = qualitative_feedback.get("top_improvements", [])
            if top_improvements:
                st.markdown("<strong>Top improvements:</strong>", unsafe_allow_html=True)
...
Status: {decision_band.replace("_", " ").title() if decision_band else ("✅ Shortlisted" if selected else "❌ Not Selected")}

## Analysis Reasoning
{analysis_result.get('reasoning', 'No reasoning provided.')}

## Qualitative Recruiter Review
Verdict: {qualitative_feedback.get('verdict', 'N/A')}
Confidence: {qualitative_feedback.get('confidence', 'N/A')}
Summary: {qualitative_feedback.get('summary', 'No qualitative summary provided.')}
        ...
        if generate_questions_func:
            with st.spinner("Generating personalized interview questions..."):
                ...
                for i, item in enumerate(questions):
                    # Support both tuple format: (type, question) and plain string format.
                    if isinstance(item, (list, tuple)) and len(item) >= 2:
                        q_type, question = item[0], item[1]
                    else:
                        q_type, question = "General", str(item).strip()
                    if not question:
                        continue
        ...
        if improve_resume_func:
            with st.spinner("Analyzing and generating improvements..."):
                ...
                for area, suggestions in improvements.items():
                    # Support both rich dict format and simple list/string format.
                    if isinstance(suggestions, dict):
                        description = suggestions.get("description", f"Suggestions for improving {area.lower()}.")
                        specific = suggestions.get("specific", [])
                        before_after = suggestions.get("before_after")
                    elif isinstance(suggestions, list):
                        description = f"Suggestions for improving {area.lower()}."
                        specific = [str(s).strip() for s in suggestions if str(s).strip()]
                        before_after = None
                    else:
                        description = f"Suggestions for improving {area.lower()}."
                        specific = [str(suggestions).strip()] if str(suggestions).strip() else []
                        before_after = None
                    ...
                    with st.expander(f"Improvements for {area}", expanded=True):
                        ...
                        for i, suggestion in enumerate(specific):
                        ...
                        if before_after:
                            ...
                            st.markdown(f"<pre>{before_after.get('after', '')}</pre>", unsafe_allow_html=True)
                            st.markdown('</div>', unsafe_allow_html=True)
                    ...
                    download_content += f"{description}\n\n"
                    ...
                    if before_after:
                        ...
                        download_content += f"```\n{before_after.get('before', '')}\n```\n\n"
                        download_content += "### After\n\n"
                        download_content += f"```\n{before_after.get('after', '')}\n```\n\n"
