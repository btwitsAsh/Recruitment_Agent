...
def analyze_resume(agent, resume_file, role, custom_jd):
    ...
    try:
        with st.spinner("🔍 Analyzing resume... This may take a minute."):
            if custom_jd:
                result = agent.analyze_resume(resume_file, custom_jd=custom_jd, role_name=role)
            else:
...
def cleanup():
    ...
    # atexit may run without a stable Streamlit session context
    resume_agent = st.session_state.get("resume_agent", None)
    if resume_agent:
        resume_agent.cleanup()
